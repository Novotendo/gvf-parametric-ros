...
typedef struct {
  double a, b, c, d, x_ini;
  // f(x) = a + b*(x-x_ini) + c*(x-x_ini)^2 + d*(x-x_ini)^3
} spline_coeff;
...
public:
    ...
    spline_coeff get_spline_coeff (double x) const;  
...
spline_coeff spline::get_spline_coeff(double x) const
{
    // Returns the corresponding spline parameters from the set (based on "operator ()" function)
    ...
    coefficients.x_ini = m_x[idx];

    if(x<m_x[0]) {
        // extrapolation to the left
        // interpol=(m_c0*h + m_b[0])*h + m_y[0];

        coefficients.a = m_y[0];
        coefficients.b = m_b[0];
        coefficients.c = m_c0;
        coefficients.d = 0.0;
        ...
        coefficients.a = m_y[n-1];
        coefficients.b = m_b[n-1];
        coefficients.c = m_c[n-1];
        coefficients.d = 0.0;
        ...
        coefficients.a = m_y[idx];
        coefficients.b = m_b[idx];
        coefficients.c = m_c[idx];
        coefficients.d = m_d[idx];
    }
    //return interpol;
